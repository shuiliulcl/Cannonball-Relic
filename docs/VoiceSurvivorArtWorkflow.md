...
- Damaged HP bars must be positioned from the visual sprite footprint, not from the gameplay collision radius. Enlarged glowy enemy sprites need a top-side UI reserve zone so bars do not cut through cue arcs, rings, or silhouettes.
...
- If a sprite family still collapses under combat density, prefer a tiny runtime cue layer over regenerating the whole sheet immediately. Cues should describe gameplay role: direction, mass, firing lens, repeat pulse, silence aura, or priority ring. Draw cue layers under the sprite or outside the body first; avoid placing a cue directly on top of the readable monster silhouette.
- Avoid duplicated cue language. If the sprite body already uses dots, do not use orbit dots as the cue; switch to segmented arcs, compression curves, wakes, or broken rings. If the sprite body is directional, rotate it to the movement or aim vector before adding extra direction cues.
- Player bodies and enemy bodies follow different direction rules. If the player sprite/body does not rotate during normal movement, its base silhouette must be round or radially balanced; put aiming and launch direction into VFX layers instead of a nose, fin, cloak point, or rocket-like body.
- Player readability in dense waves should be tested as a stack: local contrast pocket, bright center core, radially balanced locator ring, skill VFX, and HUD opacity. Avoid fixing density only by scaling the player sprite, because scale changes can imply a larger collision body.
- Player-adjacent Buff readability should stay capped and layered. Use outer VFX to show function and a small inner orbit to show active status; when many Buffs are active, prioritize shield/cannon/focus and major projectile modifiers instead of drawing every modifier at equal weight.
- If player Buff states still read as color-only variants, promote them from status dots into action-shaped signatures. Defense should wrap, control should crystallize or reticle, projectile modifiers should show fan/spear/bounce geometry, and damage should burst or slash. Review these in runtime-scale screenshots where hue is not the only separator.
- Use `?gm=1` on the survivor route for art review. The GM panel can start a run, open upgrade drafts, grant guard turret Buffs, unlock SkillGo, place temporary turrets, and trigger a full Buff showcase so screenshots do not depend on random level-up drafts.
- When an enemy family still fails after cue tuning, regenerate that family as a small controlled procedural PNG set before regenerating the entire sheet. Keep source copies and a manifest beside the runtime override.
